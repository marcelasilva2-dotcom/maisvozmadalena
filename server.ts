...
import { INITIAL_REPORTS, SECRETARIATS } from "./src/data/mockData";
...
app.get("/api/reports/protocol/:protocol", (req, res) => {
  const protocolInput = req.params.protocol.trim().toUpperCase();
  const report = reportsDatabase.find(r => r.protocol.toUpperCase() === protocolInput);
